...
class FFInstagram extends FFBaseFeed {
	...
	private function getUserId($content, $accessToken){
		...
		if (!is_object($json) || (is_object($json) && sizeof($json->data) == 0)) {
			...
			else $this->errors[] = array('type'=>'instagram', 'message' => 'Bad request, access token issue', 'url' => "https://api.instagram.com/v1/users/search?q={$content}&access_token={$accessToken}");
			return $content;
		}
		else {
